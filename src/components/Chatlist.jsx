...
const Chatlist = ({ setSelectedUser }) => {
    ...
    // Get current user's document (for loginMethod)
    useEffect(() => {
                ...
                {sortedChats?.map((chat) => (
                    <button key={chat?.id} className="flex items-start justify-between w-[100%] border-b border-[#9090902c] px-5 pb-3 pt-3">
                        {chat?.users
                            ...
                            // Apply the phone vs email/google filter
                            ?.filter((u) => {
                                if (!currentUserDoc) return false;
                                if (currentUserDoc.loginMethod === "phone") {
                                    return u.loginMethod === "google" || u.loginMethod === "email";
                                }
                                return true;
                            })
                            ?.map((u) => (
